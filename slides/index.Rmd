...
ess <- ess %>% 
  left_join(cntry_dat %>% 
  select(cntry, gini_coef = c_gini_2018))
...
lvl2_preds_model <- lmer(income ~ education * gini_coef + (1|cntry), data = ess)

```

```{r, echo = T, results='asis', eval=F}
lvl2_preds_model <- lmer(income ~ education `+ cpi +` (1|cntry), data = ess)
```

```{r}
summary(lvl2_preds_model)

texreg::screenreg(lvl2_preds_model)

sjPlot::plot_model(lvl2_preds_model, type = "int")
```
...
The variance for the slope is $0.0114$.
...
In this case, we want AIC and BIC values to be closer to zero.
